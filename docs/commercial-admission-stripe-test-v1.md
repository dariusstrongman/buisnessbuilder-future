...
A delayed admission does not become pay-now merely because its `eligible_at` timestamp passes. A newly signed, current appointed-operator release may supersede it only after that timestamp, against the unchanged order/quote digest and a distinct decision reference. The prior admission remains immutable and linked from the new record; retrying the exact new decision is idempotent. This is a technical waiting-period control, not a legal conclusion about whether or how long a delay is required.
...
## Actual Stripe sandbox acceptance (no funds moved)

The replacement key in AWS Secrets Manager authenticated to the isolated **Business Builder Pilot Test sandbox** account. Its value and the CLI-issued webhook signing secrets were never printed or committed. The scripts in this branch require a loopback-only disposable PostgreSQL DSN and reject a credential for a different sandbox account. An official Stripe CLI Docker image forwarded real sandbox events to a bounded local HTTP listener; the listener verified the raw-body signature before using the existing Commercial ingress. Neither a persistent Stripe webhook endpoint nor standing AWS infrastructure was created.

| Test-mode case | Observed result |
|---|---|
| Build My Business | Hosted Checkout showed $1,495; fake-card completion produced a signed paid event and an exact PostgreSQL order/entitlement transition. |
| Build My Business + Run | Hosted Checkout charged $1,995 upfront plus the first $299 cycle; a signed event persisted the subscription and entitlement. |
| Quote-backed Existing Business + Run | A separate unpriced audit/digest-backed order, exact $1,695 test quote, founder approval and appointed test operator release led to a $1,994 first charge (setup + first $299 cycle). The marketing floor alone never generated Checkout. |
| Immediate card decline | Stripe emitted `payment_intent.payment_failed`, but Checkout remained open for card retry and granted nothing. Expiring that exact unpaid session produced a signed `checkout.session.expired` event and a canceled PostgreSQL order. |
| Refund | A real test refund of the fixed build produced signed `refund.created`; one refund record and a refunded order persisted. |
| Cancellation | A real test subscription cancellation produced signed `customer.subscription.deleted`; managed entitlements expired, customer-owned export/read grants remained active. |
| Recurring success and failure | A synthetic sandbox customer and Stripe test clock produced a real $299 `subscription_cycle` invoice success followed by a real $299 failed cycle using Stripe's attachable decline-after-attach test method. Signed invoice events were applied once. Managed AI Workforce grants became `EXPIRING` after failure; the existing Runtime entitlement gate denied a new Inbox Assistant job while customer-owned grants survived. |
| Authenticity, replay, restart, isolation | A forged callback returned 400. Exact real signed refund, cancellation, expiry, renewal and Checkout events were immediately replayed and made zero further changes. Reopened PostgreSQL repositories retained the orders, subscriptions, entitlements and audit; wrong-tenant listings returned nothing. |

The test-clock Checkout uses a **test-script-only** pre-created Customer transport variation to make renewal time travel possible. It does not change the production Stripe adapter or public contract. The script operator identity/appointment and existing-business digest are synthetic test fixtures; separate backend tests prove the real cited residential-cleaning scope and browser contract. This acceptance does **not** prove a unified live Cognito founder/operator session through the flagship site into this Stripe account. It also does not approve tax treatment, FTC/disclosure timing, live charging, or a production webhook endpoint. [Stripe sandbox testing](https://docs.stripe.com/testing), [test clocks](https://docs.stripe.com/billing/testing/test-clocks), and [billing failure testing](https://docs.stripe.com/billing/testing) describe the no-funds test mechanisms used here.
...
Separate live blockers: verified terms/refund/cancellation disclosures, founder-facing quote/release approval procedure, a privileged operator appointment provisioner, production tax-authority adapter, a unified Cognito/BFF-to-this-sandbox acceptance run, live webhook endpoint configuration, supervised monitoring, and explicit live-charge approval. The isolated Stripe test-account event/Checkout authority is proven above; none of the separate legal or live-launch gates is silently cleared.

## Test-only environment names

`BUSINESSBUILDER_TEST_POSTGRES_DSN`, `BUSINESSBUILDER_TEST_POSTGRES_SCHEMA`, `BUSINESS_BUILDER_API_URL`, `BUSINESS_BUILDER_STRIPE_TEST_SECRET_REF`, `BUSINESS_BUILDER_STRIPE_TEST_ACCOUNT_ID`, `BUSINESS_BUILDER_STRIPE_TEST_WEBHOOK_SECRET_REF`, `BUSINESS_BUILDER_STRIPE_SANDBOX_SCHEMA`, `BUSINESS_BUILDER_STRIPE_SANDBOX_REFERENCE_SESSION`, `BUSINESS_BUILDER_STRIPE_SANDBOX_CLOCK_CUSTOMER_REF`, `BUSINESS_BUILDER_STRIPE_SANDBOX_EVENTS`, `BUSINESS_BUILDER_STRIPE_SANDBOX_EXPECT_TYPE`, `BUSINESS_BUILDER_STRIPE_SANDBOX_EXPECT_COUNT`, `BUSINESS_BUILDER_CHECKOUT_SUCCESS_URL`, `BUSINESS_BUILDER_CHECKOUT_CANCEL_URL`, `BUSINESS_BUILDER_TEST_AUTH_MODE` (local harness only). Store persistent values only in the approved secret/deployment store. Do not paste credentials into chat or command output. The earlier test key pasted in chat should be revoked; the replacement key used here was read from Secrets Manager without disclosure.
